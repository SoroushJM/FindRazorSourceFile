...
const getConfigValue = (
  configs: FindRazorSourceFileConfig[],
  key: string,
  defaultValue: string,
): string => {
...
const combineRects = (rects: Rect[]): Rect =>
  rects.reduce(
    (pre, cur) => ({
...
const applyStyle = (element: HTMLElement, style: object) =>
...
const createElement = (
  ...
  style?: object | null,
  attrib?: object | null,
  children?: (CreateElementResult | { [key: string]: CreateElementResult })[],
): CreateElementResult => {
...
const createComponentsMap = async (): Promise<ComponentMap[]> => {
  const detectMarker = (node: Node) =>
    node.textContent?.trim().match(/^(begin|end):(frsf-[a-z0-9]{10})$/) || [];
      ...
      if (sibling.nodeType === COMMENT_NODE) {
        ...
        if (endTag === "end" && endHash === beginHash && elements.length > 0) {
          componentsMap.push({
            hash: beginHash,
            rect: () =>
              combineRects(elements.map((e) => e.getBoundingClientRect())),
...
class UIRoot extends HTMLElement {
  ...
  connectedCallback() {
    ...
    addEventListener(ui.settingsOpenInVSCode, {
      click: settingsOpenInVSCode_onClick,
    });
    ...
    this.dispose = () => {
      ...
      removeEventListener(ui.settingsOpenInVSCode, {
        click: settingsOpenInVSCode_onClick,
      });
    };
  }
...
const createUIElements = (parent: ShadowRoot): UIElements => {
  const [overlay, exposes] = createElement(
    "div",
    {
      position: "fixed",
      top: "0",
      ...
      opacity: "0",
      transition:
        "border 0.2s ease-out, box-shadow 0.2s ease-out, opacity 0.2s linear",
    },
    NULL,
    [
      {
        sourceNameTip: createElement(
          "div",
          {
            position: "absolute",
            top: "4px",
            ...
            boxShadow: "2px 2px 4px 0px rgb(0, 0, 0, 0.5)",
            whiteSpace: "nowrap",
            display: none,
          ...
          [
            createElement(
              ...
              { src: CONTENT_ROOT + "ASPWebApplication_16x.svg" },
            ),
            {
              sourceNameTipProjectName: createElement("span", {
            ...
            createElement(
              "img",
              { verticalAlign: "middle", width: "16px" },
              { src: CONTENT_ROOT + "ASPRazorFile_16x.svg" },
            ),
            {
              sourceNameTipItemName: createElement("span", {
                verticalAlign: "middle",
                marginLeft: "4px",
              }),
            },
      ...
      },
      {
        settingsButton: createElement(
          ...
          {
            title: "Find Razor Source File - Settings",
            textContent: "Find Razor Source File",
          },
        ),
      },
      {
        settingsForm: createElement(
          ...
          [
            createElement(
              ...
              [
                {
                  settingsOpenInVSCode: createElement(
                    "input",
                    {
                      margin: "0 8px 0 0",
                      padding: "0",
                ...
                createElement(
                  ...
                  {
                    textContent:
                      "Open the .razor file of the clicked component in ",
                  },
                ),
                createElement(
                  "img",
                  { verticalAlign: "middle", width: "18px" },
                  { src: CONTENT_ROOT + "vscode.svg" },
                ),
                createElement(
                  "span",
                  { verticalAlign: "middle" },
...
const onKeyDown = async (ev: KeyboardEvent): Promise<void> => {
  ...
  if (currentMode === Mode.Inactive && pressedHotkey) {
    ...
    hideSettingsForm();
    setTimeout(() => {
      if (currentMode === Mode.Active || currentMode === Mode.Locked)
        uiElements.overlay.style.opacity = "1";
    }, 1);
    setSourceNameTip("", "");
    currentScope = NULL;
    ...
    (currentMode === Mode.Active || currentMode === Mode.Locked) &&
    (pressedEscape || pressedHotkey)
  ) {
    stopPropagation(ev);
    ev.preventDefault();

    currentMode = pressedHotkey
      ? Mode.Inactive
...
const isPressedHotkey = (
  configs: FindRazorSourceFileConfig[],
  ev: KeyboardEvent,
): boolean => {
  const getHotleyConfigValue = (key: string, defaultValue: string): string =>
    getConfigValue(configs, "hotkey:" + key, defaultValue);
  return (
    ev.code === getHotleyConfigValue("code", "KeyF") &&
...
const overlay_onClick = (ev: MouseEvent): void => {
  ...
  if (currentMode === Mode.Active) {
    if (
      currentScope &&
      lastDetectedRazorSource &&
      lastDetectedRazorSource !== NotFound
    ) {
      currentMode = Mode.Locked;
      updateUIeffects(Mode.Locked);
      const event = new Event(RazorSourceEventNames.LockIn, {
        bubbles: false,
        cancelable: false,
      }) as RazorSourceEvent;
...
const showSettingsForm = (): CSSStyleDeclaration =>
  Object.assign(uiElements.settingsForm.style, {
    opacity: "1",
    bottom: "48px",
    pointerEvents: "unset",
  });

const hideSettingsForm = (): CSSStyleDeclaration =>
  Object.assign(uiElements.settingsForm.style, {
    opacity: "0",
    bottom: "0",
    pointerEvents: none,
...
const isHiddenSettingsForm = (): boolean =>
  uiElements.settingsForm.style.opacity === "0";
...
const detectScope = async (ev: MouseEvent): Promise<DetectedScopeResult> => {
  ...
  if (scope && topElement) {
    ...
    scopeRect =
      (currentScope === scope ? currentScopeRect : NULL) ||
      getScopeRect(topElement);
  ...
  if (hoveredComponentMap) {
    ...
    if (!scope) {
      ...
      if (
        scopeSource !== NotFound &&
        scopeSource.itemName === hoveredComponentMap.source.itemName
      ) {
        scopeRect = combineRects([
          scopeRect || MaxRect,
          hoveredComponentMap.rect(),
        ]);
  ...
  // else, next scope is found and current scope is also available...
    ...
    if (isPointerInRect(ev, currentScopeRect)) {
      ...
      if (
        scopeRect.left < currentScopeRect.left &&
...
const getScope = async (element: Element): Promise<string | null> => {
  const scope =
    element.getAttributeNames().filter((name) => name.startsWith("b-"))[0] ||
    NULL;
...
const getRazorSourceName = async (
  arg: string | DetectedScopeResult | null,
): Promise<RazorSourceName | null> => {
  ...
  const res = await fetch(`./RazorSourceMapFiles/${scope}.txt`)
    .then((r) =>
      r.ok ? r : fetch(`${CONTENT_ROOT}RazorSourceMapFiles/${scope}.txt`),
    )
    .catch((e) => console.error(e));
  if (res && res.ok) {
    const text = await res.text();
    const p = text.replace(/[\r\n]*$/gi, "").split("|");
    const razorSourceName = {
      projectName: p[0],
      itemName: p[1],
      fullPath: p[2],
    };
...
const displayScopeMask = (
  scopeRect: Rect | null,
  razorSourceName: RazorSourceName | null,
): void => {
...
const getScopeRect = (element: Node): Rect => {
  ...
  const getRect = (children: Node[]): Rect =>
    combineRects(
      children.map((e) =>
        e.nodeType === ELEMENT_NODE
          ? (e as HTMLElement).getBoundingClientRect()
          : e.nodeType === COMMENT_NODE
            ? getRect(getLogicalNodes(e)[1])
            : MaxRect,
      ),
    );
...
const window_onResize = (ev: UIEvent): void => {
  ...
  currentScopeRect = combineRects(
    currentScopeElements.map((e) => getScopeRect(e)),
  );
  displayScopeMask(currentScopeRect, lastDetectedRazorSource);
};

const window_onStorage = (ev: StorageEvent): void =>
  loadOptionsFromLocalStorage();
...
const loadOptionsFromLocalStorage = (): void => {
  const optionString = localStorage.getItem(
    FindRazorSourceFileClientOptionsKey,
  );
